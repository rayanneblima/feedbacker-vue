function init (apiKey) {
  async function handleLoadWidget () {
    ...
    const WIDGET_URL = `https://rayanneblima-feedbacker-widget.vercel.app?api_key=${apiKey}&page=${page}&fingerprint=${fingerprint.visitorId}`
    const config = { method: 'HEAD' }
    const res = await fetch(`https://backend-vue-feedbacker.vercel.app/apikey/exists?apikey=${apiKey}`, config)
